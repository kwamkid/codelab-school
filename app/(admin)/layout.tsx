...
import { MakeupProvider, useMakeup } from '@/contexts/MakeupContext';
...
// Inner component ที่ใช้ context
function AdminLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  // ใช้ context แทน state
  const { pendingMakeupCount } = useMakeup();
  
  // Simple notification count
  const [notificationCount, setNotificationCount] = useState(0);

  // Simple notification check - just count makeup notifications
  useEffect(() => {
    const checkNotifications = async () => {
      if (user) {
        try {
          const makeupClasses = await getMakeupClasses();
          const autoGenerated = makeupClasses.filter(
            m => m.requestedBy === 'system' && m.status === 'pending'
          ).length;
          setNotificationCount(autoGenerated);
        } catch (error) {
                ...
                {item.subItems ? (
                  ...
                  >
                    <div className="flex items-center justify-between w-full">
                      ...
                      {item.badge && (
                        <span className="ml-auto inline-flex items-center justify-center h-5 min-w-[20px] px-1.5 text-xs font-medium text-white bg-red-500 rounded-full animate-pulse">
                          {item.badge}
                        </span>
                      )}
                    </div>
