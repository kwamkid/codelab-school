...
export default function LiffHomePage() {
  ...
  const { parent, students, loading: parentLoading } = useLiffParent();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-gradient-to-r from-red-500 to-red-600 text-white p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-red-100">โรงเรียนสอนเขียนโปรแกรม</p>
          </div>
          {profile?.pictureUrl && (
            <Image
        ...
        <div className="bg-white/10 rounded-lg p-3">
          ...
          {isLoggedIn && profile ? (
            <p className="text-red-100">คุณ{profile.displayName}</p>
          ) : (
            <div className="flex items-center gap-2 mt-1">
              <p className="text-red-100 text-sm">ยังไม่ได้เข้าสู่ระบบ</p>
              <Button
                size="sm"
                variant="secondary"
                onClick={() => liff?.login()}
        ...
        {isLoggedIn && students.length > 0 && (
          <div className="mt-4 space-y-2">
            <p className="text-sm text-red-100">นักเรียนในความดูแล</p>
              ...
              >
                <Card className="h-full hover:shadow-md transition-shadow">
                  <CardContent className="p-4">
                    <div className={`${item.bgColor} w-12 h-12 rounded-lg flex items-center justify-center mb-2`}>
                    ...
                    {item.requireLogin && !isLoggedIn && (
                      <Badge variant="outline" className="absolute bottom-2 right-2 text-xs px-1.5 py-0.5">
          ...
          <CardContent className="p-4">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <h3 className="font-medium mb-1">ประกาศ</h3>
      ...
      <div className="px-4 pb-6">
        <h2 className="font-medium text-gray-700 mb-3">ติดต่อเรา</h2>
        <div className="space-y-2">
          {quickContacts.map((contact) => (
            <Card key={contact.title}>
              <CardContent className="p-4">
                <h3 className="font-medium text-sm mb-2">{contact.title}</h3>
